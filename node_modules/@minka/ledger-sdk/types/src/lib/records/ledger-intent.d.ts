...
import { LedgerMeta, LedgerSignatureCustom } from '../common/ledger-meta';
 ...
 */
export type IntentChangeMeta = MergeExclusive<ChangeMeta<IntentSignatureData>, IntentMeta>;
/**
 * Ledger metadata.
 */
export type IntentMeta = LedgerMeta<IntentSignatureData> & {
    /**
 ...
 * to an intent.
 */
export type IntentSignatureData = LedgerSignatureCustom & {
    /**
     ...
     * signature.
     */
    status?: IntentStatus | 'pending' | 'prepared' | 'requested' | 'failed' | 'committed' | 'completed' | 'aborted' | 'rejected' | 'created' | 'error' | 'expired';
    /**
     ...
     * of action fails in external system. Should be added
     * if `status` is 'failed' and must be one of predefined error reasons.
     */
