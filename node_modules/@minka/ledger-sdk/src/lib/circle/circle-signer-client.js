...
class CircleSignerClient extends base_client_1.BaseClient {
    ...
    init(record) {
        const builder = new create_record_builder_1.CreateRecordBuilder(async (record, authParams) => {
            return this.createRecord(this.buildBasePath(), CircleSignerResponse, record, authParams);
        });
    ...
    drop(id) {
        ...
        const builder = new drop_record_builder_1.DropRecordBuilder(async (record, authParams) => {
            return this.dropRecord(`${this.buildBasePath()}/${encodeURIComponent(signerIdentifier)}`, record, authParams);
        }, async (authParams) => {
            const response = await this.read(signerIdentifier, authParams);
            return {
    ...
    async read(id, authParams) {
        return this.getRecord(`${this.buildBasePath()}/${encodeURIComponent(id)}`, CircleSignerResponse, authParams);
    }
    /**
