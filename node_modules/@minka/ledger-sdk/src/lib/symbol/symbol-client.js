...
class SymbolClient extends base_client_1.BaseClient {
    ...
    init(record) {
        const builder = new create_record_builder_1.CreateRecordBuilder(async (record, authParams) => {
            return this.createRecord(SYMBOLS_PATH, SymbolResponse, record, authParams);
        });
    ...
    edit(record) {
        const builder = new update_record_builder_1.UpdateRecordBuilder(async (record, authParams) => {
            return this.updateRecord(`${SYMBOLS_PATH}/${encodeURIComponent(record.data.handle)}`, SymbolResponse, record, authParams);
        });
        record = lodash_1.default.cloneDeep(record);
        return builder.init(record);
    }
    ...
    async read(id, authParams) {
        return this.getRecord(`${SYMBOLS_PATH}/${encodeURIComponent(id)}`, SymbolResponse, authParams);
    }
    /**
