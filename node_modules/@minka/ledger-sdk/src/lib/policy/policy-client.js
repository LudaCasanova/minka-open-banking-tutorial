...
exports.PolicyClient = exports.PoliciesResponse = exports.PolicyResponse = void 0;
 ...
 */
class PolicyResponse extends base_record_response_1.BaseRecordResponse {
    /**
 ...
 */
class PoliciesResponse extends list_response_1.BaseListResponse {
    policies;
    constructor(response) {
        super(response);
        this.policies = response?.data?.data?.map((record) => record.data);
    }
}
 ...
 * @see {LedgerPolicy}
 */
class PolicyClient extends base_client_1.BaseClient {
    /**
    ...
    constructor(options) {
        super(options);
        this.change = new base_change_client_1.BaseChangeClient(options, POLICIES_PATH);
    ...
    init(record) {
        const builder = new create_record_builder_1.CreateRecordBuilder(async (record, authParams) => {
            return this.createRecord(POLICIES_PATH, PolicyResponse, record, authParams);
        });
    ...
    edit(record) {
        const builder = new update_record_builder_1.UpdateRecordBuilder(async (record, authParams) => {
            return this.updateRecord(`${POLICIES_PATH}/${encodeURIComponent(record.data.handle)}`, PolicyResponse, record, authParams);
        });
        record = lodash_1.default.cloneDeep(record);
        return builder.init(record);
    }
    ...
    async read(id, authParams) {
        return this.getRecord(`${POLICIES_PATH}/${encodeURIComponent(id)}`, PolicyResponse, authParams);
    }
    /**
     ...
     */
    async list(params) {
        return this.getRecordsList(POLICIES_PATH, PoliciesResponse, params);
    }
}
