...
exports.AnchorClient = exports.AnchorsResponse = exports.AnchorResponse = void 0;
 ...
 */
class AnchorResponse extends base_record_response_1.BaseRecordResponse {
    /**
    ...
    constructor(response) {
        super(response);
        this.anchor = response?.data?.data;
    }
}
exports.AnchorResponse = AnchorResponse;
/**
 * Anchors list API response.
 */
class AnchorsResponse extends list_response_1.BaseListResponse {
    /**
...
exports.AnchorsResponse = AnchorsResponse;
/**
 * Allows querying and managing anchors.
 *
 * @see {LedgerAnchor}
 */
class AnchorClient extends base_client_1.BaseClient {
    /**
    ...
    constructor(options) {
        super(options);
        this.change = new base_change_client_1.BaseChangeClient(options, ANCHORS_PATH);
        this.links = [this.change];
    }
    /**
    ...
    init(record) {
        const builder = new create_record_builder_1.CreateRecordBuilder(async (record, authParams) => {
            return this.createRecord(ANCHORS_PATH, AnchorResponse, record, authParams);
        });
    ...
    edit(record) {
        const builder = new update_record_builder_1.UpdateRecordBuilder(async (record, authParams) => {
            return this.updateRecord(`${ANCHORS_PATH}/${encodeURIComponent(record.data.handle)}`, AnchorResponse, record, authParams);
        });
        record = lodash_1.default.cloneDeep(record);
        return builder.init(record);
    }
     ...
     */
    drop(id) {
        const anchorIdentifier = id || this.id;
        const builder = new drop_record_builder_1.DropRecordBuilder(async (record, authParams) => {
            return this.dropRecord(`${ANCHORS_PATH}/${encodeURIComponent(anchorIdentifier)}`, record, authParams);
        }, async (authParams) => {
            const response = await this.read(anchorIdentifier, authParams);
            return {
                data: response.anchor,
                hash: response.hash,
                luid: response.luid,
                meta: response.meta,
            };
    ...
    async read(id, authParams) {
        return this.getRecord(`${ANCHORS_PATH}/${encodeURIComponent(id)}`, AnchorResponse, authParams);
    }
    /**
     ...
     */
    async list(params) {
        return this.getRecordsList(ANCHORS_PATH, AnchorsResponse, params);
    }
}
exports.AnchorClient = AnchorClient;
