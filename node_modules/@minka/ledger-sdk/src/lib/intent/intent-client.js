...
class IntentClient extends base_client_1.BaseClient {
    ...
    init(record) {
        const builder = new create_record_builder_1.CreateRecordBuilder(async (record, authParams) => {
            return this.createRecord(INTENTS_PATH, IntentResponse, record, authParams);
        });
    ...
    from(record) {
        record = lodash_1.default.cloneDeep(record);
        return new sign_intent_builder_1.SignIntentBuilder(async (signature, authParams) => {
            return this.signRecord(`${INTENTS_PATH}/${encodeURIComponent(record.data.handle)}/proofs`, IntentResponse, signature, authParams);
        }, record);
    }
    /**
    ...
    async read(id, authParams) {
        return this.getRecord(`${INTENTS_PATH}/${encodeURIComponent(id)}`, IntentResponse, authParams);
    }
    /**
