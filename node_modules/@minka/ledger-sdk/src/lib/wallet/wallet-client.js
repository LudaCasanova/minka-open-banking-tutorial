...
class WalletClient extends base_client_1.BaseClient {
    ...
    constructor(options) {
        ...
        this.links = [this.anchor, this.change];
    }
    /**
    ...
    init(record) {
        const builder = new create_record_builder_1.CreateRecordBuilder(async (record, authParams) => {
            return this.createRecord(WALLETS_PATH, WalletResponse, record, authParams);
        });
    ...
    edit(record) {
        const builder = new update_record_builder_1.UpdateRecordBuilder(async (record, authParams) => {
            return this.updateRecord(`${WALLETS_PATH}/${encodeURIComponent(record.data.handle)}`, WalletResponse, record, authParams);
        });
        record = lodash_1.default.cloneDeep(record);
        return builder.init(record);
    }
    ...
    drop(id) {
        ...
        const builder = new drop_record_builder_1.DropRecordBuilder(async (record, authParams) => {
            return this.dropRecord(`${WALLETS_PATH}/${encodeURIComponent(WalletIdentifier)}`, record, authParams);
        }, async (authParams) => {
            const response = await this.read(WalletIdentifier, authParams);
            return {
    ...
    async read(id, authParams) {
        return this.getRecord(`${WALLETS_PATH}/${encodeURIComponent(id)}`, WalletResponse, authParams);
    }
    /**
